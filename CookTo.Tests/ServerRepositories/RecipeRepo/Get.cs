...
public class Get
{
	...
	[Fact]
	public async Task GetAllRecipeAsync_ExceptionThrown_Success()
	{
		//Mock MoveNextAsync
		Mock<IAsyncCursor<Recipe>> _RecipeCursor = new Mock<IAsyncCursor<Recipe>>();
		_RecipeCursor.Setup(_ => _.Current).Returns(_list);
		_RecipeCursor.SetupSequence(_ => _.MoveNext(It.IsAny<CancellationToken>()))
			.Returns(true)
			.Returns(false);

		//Mock FindAsync
		_mockCollection.Setup(op => op.FindAsync(It.IsAny<FilterDefinition<Recipe>>(),
			It.IsAny<FindOptions<Recipe, Recipe>>(),
			It.IsAny<CancellationToken>()))
			.ThrowsAsync(new Exception());

		//Mock GetCollection
		_mockContext.Setup(c => c.GetCollection<Recipe>(typeof(Recipe).Name)).Returns(_mockCollection.Object);
		var RecipeRepo = new RecipeRepository(_mockContext.Object);

		var result = () =>  RecipeRepo.GetAllAsync();
		await Assert.ThrowsAsync<Exception>(result);



		//verify the InsertOneAsync is called once
		_mockCollection.Verify(c => c.FindAsync(It.IsAny<FilterDefinition<Recipe>>(),
	...
	[Fact]
	public async Task GetAllRecipeAsync_NoDocumentsDound_Returned_Null_Success()
	{
		//Mock MoveNextAsync retu
		Mock<IAsyncCursor<Recipe>> _RecipeCursor = new Mock<IAsyncCursor<Recipe>>();
		_RecipeCursor.Setup(_ => _.Current).Returns(new List<Recipe>());
		_RecipeCursor.SetupSequence(_ => _.MoveNext(It.IsAny<CancellationToken>()))
			.Returns(true)
			.Returns(false);
		...
		_mockCollection.Setup(op => op.FindAsync(It.IsAny<FilterDefinition<Recipe>>(),
			It.IsAny<FindOptions<Recipe, Recipe>>(),
			It.IsAny<CancellationToken>()))
			.ReturnsAsync(_RecipeCursor.Object);

		//Mock GetCollection
		_mockContext.Setup(c => c.GetCollection<Recipe>(typeof(Recipe).Name)).Returns(_mockCollection.Object);
		var RecipeRepo = new RecipeRepository(_mockContext.Object);

		var result = await RecipeRepo.GetAllAsync();
